...
- Supports common PMDG MSFS 2024 livery package structures, including ZIP-based `.ptp` packages and full `*-liveries` packages.
...
```text
PMDG Livery Installer MSFS2024 Setup v0.1.3.exe
```
...
It then rebuilds that livery package `layout.json` automatically, replacing the manual drag-to-layout-generator step.
...
- 支持常见 PMDG MSFS 2024 涂装包结构，包括 ZIP-based `.ptp` 包和完整 `*-liveries` 包。
- 安装后自动重建 `layout.json`。
- 在适用时更新包元数据。
- 默认备份原始 `layout.json`。
...
随后会自动重建该 livery 包的 `layout.json`，无需手动把 `layout.json` 拖到 layout generator 上。
