...
- `Manage` page for reviewing installed liveries, previewing thumbnails, and uninstalling selected liveries.
- `Install` page for installing compatible ZIP or folder liveries.
...
```text
PMDG Livery Installer MSFS2024 Setup v0.1.51.exe
```
...
1. Open the app.
2. Confirm or detect the MSFS 2024 Community folder on `Install`.
3. Select the target PMDG aircraft package.
4. Select a livery ZIP or extracted folder.
5. Review install options, then click `Install Livery`.
...
Use the `Manage` page to scan the selected aircraft's companion `*-liveries` package. Selecting a livery shows its folder, metadata, file counts, size, and the first supported thumbnail found under the livery folder. `Uninstall Selected` removes only that selected livery folder and rebuilds the livery package `layout.json`.

## Safety Notes

- This v0.1.51 update has not been verified in a real MSFS 2024/PMDG installation environment. Back up your relevant Community `pmdg-aircraft-*-liveries` package before using install or uninstall features.
...
- `Manage` 页面：查看已安装涂装、预览缩略图，并卸载选中的涂装。
- `Install` 页面：安装兼容的 ZIP 或文件夹涂装。
...
1. 打开程序。
2. 在 `Install` 页面确认或自动探测 MSFS 2024 Community 文件夹。
3. 选择目标 PMDG 机模包。
4. 选择涂装 ZIP 或已解压文件夹。
5. 检查安装选项，然后点击 `Install Livery`。
...
进入 `Manage` 页面后选择机型并扫描配套的 `*-liveries` 包。点击涂装会显示文件夹、元数据、文件数量、大小和可识别的缩略图。点击 `Uninstall Selected` 只会删除当前选中的单个涂装文件夹，并重新生成该 livery 包的 `layout.json`。

## 安全提示

本次 v0.1.51 更新未经过真实 MSFS 2024/PMDG 安装环境的实际验证。使用安装或卸载功能前，请先备份相关 Community `pmdg-aircraft-*-liveries` 包。
